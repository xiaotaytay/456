...
class PiPManager: NSObject {
    ...
    private func drawHeroesOnCtx(_ ctx: CGContext, heroPart: String, scaleX: CGFloat, scaleY: CGFloat, canvasW: CGFloat, canvasH: CGFloat) {
        ...
        for heroStr in heroStrings {
            ...
            let pixelX = CGFloat(hero.x) * scaleX * CGFloat(heroScale) + CGFloat(globalOffsetX) + CGFloat(heroOffsetX)
            let pixelY = CGFloat(hero.y) * scaleY * CGFloat(heroScale) + CGFloat(globalOffsetY) + CGFloat(heroOffsetY)
    ...
    private func drawMonstersOnCtx(_ ctx: CGContext, monsterPart: String, scaleX: CGFloat, scaleY: CGFloat) {
        ...
        for m in monsters {
            ...
            let baseDrawX = CGFloat(m.x) * scaleX * CGFloat(monsterScale) + CGFloat(globalOffsetX) + CGFloat(monsterOffsetX)
            let baseDrawY = CGFloat(m.y) * scaleY * CGFloat(monsterScale) + CGFloat(globalOffsetY) + CGFloat(monsterOffsetY)
            let drawX = baseDrawX
            let drawY = baseDrawY
            
            if hideCountdown || m.isFullCD || m.cd == 0 {
                let baseR: CGFloat = 3.5
                let r = baseR * scaleX * CGFloat(monsterZoom)
    ...
    private func loadHeroImageAsync(_ heroId: String) {
        let shouldLoad = imageCacheQueue.sync(flags: .barrier) { () -> Bool in
